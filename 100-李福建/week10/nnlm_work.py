...
class LanguageModel(nn.Module):
    ...
    def forward(self, x, y=None):
        ...
        if y is not None:
            #mask = get_pad_mask(x, 0) & get_subsequent_mask(x)
...
def build_sample(tokenizer, window_size, corpus):
    ...
    x = tokenizer.encode(window, add_special_tokens=False, padding="max_length", max_length=len(window), truncation=True)
    y = tokenizer.encode(target, add_special_tokens=False, padding="max_length", max_length=len(target), truncation=True)
    return x, y
...
def train(corpus_path, save_weight=True):
    ...
    for epoch in range(epoch_num):
        ...
        for batch in range(int(train_sample / batch_size)):
            ...
            # print("第%d轮------第%d次" % (epoch + 1, batch + 1))
