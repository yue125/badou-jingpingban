...
# 构建字符表
def build_vocab():
    chars = "abcdefghijklmnopqrst"  #字符集
    vocab = {"pad":0}
    # 每个字对应一个序号
...
# 在字符串中找到目标字符第一次出现时的对应位置，如果不存在对应字符，返回nuk？unk 对应的下标
...
def build_sample(vocab, sentence_length):
    ...
    # 当且仅当遇到第一个 t 时，返回 t 对应的下标
    y = [find_position(x, "t", vocab['unk'])]
...
if __name__ == "__main__":
    log = main()
    predict("model.pth", "vocab.json", ["bceeeecthc", "abggggcbbt", "abggggbbct"])
    draw(log)
