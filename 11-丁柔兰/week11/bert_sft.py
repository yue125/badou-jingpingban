...
# 定义一个函数generate_sentence，用于生成文本。它接收三个参数：
# openings: 一个字符串，表示生成文本的开头
# model: 预训练的语言模型
# tokenizer: 用来编码和解码文本的分词器
def generate_sentence(openings, model, tokenizer):
    # 将模型设置为评估模式，这是推理时常用的，以关闭诸如Dropout等训练特有的层
    model.eval()
    # 使用分词器将开头文本编码成模型理解的数字序列------------------
    openings = tokenizer.encode(openings)

    # 不计算梯度，这在推理时有助于减少内存消耗并加快速度
    with torch.no_grad():
        # 只要生成的文本长度不超过50，就继续生成过程
        while len(openings) <= 50:
            # 将编码后的文本转换为LongTensor，准备输入模型
            x = torch.LongTensor([openings])
            # 如果CUDA可用，则将输入转移到GPU以加速
            if torch.cuda.is_available():
                x = x.cuda()
            # 通过模型获取预测结果，y是模型的输出
            # 这里取模型输出的最后一个token的输出，用于后续生成下一个token
            y = model(x)[0][-1]
            # 使用某种采样策略（这里未定义）从y中选择下一个token的索引
            index = sampling_strategy(y)
            # 将选中的token索引添加到openings中，为生成下一个token做准备------------------
            openings.append(index)
    # 使用分词器将编码后的文本序列解码为字符串，并返回生成的文本------------------
