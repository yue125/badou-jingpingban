...
class Max_dim_3(nn.Module):
    def __init__(self, input_size):
        super(Max_dim_3, self).__init__()
...
# 随机生成一个5维向量，进行三分类：对其中索引为0,3,4的三个数进行比较，若0号最大，则y=0;若3号最大，则y=1;若4号最大,则y=2
def build_sample():
    x = np.random.random(5)
    # 获取最大值的索引
    if max(x[0],x[3],x[4])==x[0]:
        return x, 0
    elif max(x[0],x[3],x[4])==x[3]:
        return x, 1
    else:
        return x, 2
