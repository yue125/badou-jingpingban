...
class TorchModel(nn.Module):
    def __init__(self, vector_dim, vocab):
        ...
        self.layer = nn.RNN(vector_dim, len(vocab), bias=False, batch_first=True)
        self.loss = nn.CrossEntropyLoss()

    # 当输入真实标签，返回loss值；无真实标签，返回预测值
    def forward(self, x, y=None):
        embedded_output = self.embedding(x)
        rnn_output, hidden_layer = self.layer(embedded_output)
        hidden_layer = hidden_layer.squeeze()

        if self.training:  # 训练模式，返回损失值
            y = y.squeeze()
            return self.loss(hidden_layer, y)
        else:
            return torch.softmax(hidden_layer, dim=1)
...
if __name__ == "__main__":
    log = main()
    predict("model.pt", "vocab.json", ["abcthc", "cabbbt"])
    draw(log)