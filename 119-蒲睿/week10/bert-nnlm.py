...
def build_sample(vocab, window_size, corpus):
    start = random.randint(0, len(corpus) - 1 - window_size) # 随机选取位置作为起点
    end = start + window_size # 指定窗口长度
    ...
    with torch.no_grad():
        ...
        while pred_char != "\n" and len(openings) <= 30:
            ...
            if torch.cuda.is_available():
                x = x.cuda()
            y = model(x)[0][-1] # batch的第一个元素的走后一个向量就是预测的下一个字
