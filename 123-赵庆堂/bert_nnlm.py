...
class LanguageModel(nn.Module):
    ...
    def forward(self, x, y=None):
        # 定义一个自注意力的掩码矩阵,下文的<attention_mask = mask>会自动调用
        mask_matrix = torch.ones(x.shape[0], x.shape[1], x.shape[1])
        mask = torch.tril(mask_matrix)
        # 训练时
        if y is not None:
            if torch.cuda.is_available():
                mask = mask.cuda()
            '''
            x.shape(batch_size, window_size, bert_embedding_size[代码:bert.config.hidden_size])
            y.shape(batch_size, window_size):x的后一个字
            y_pred.shape(batch_size, window_size, bert_token_vocab_size[代码:bert_tokenizer.vocab_size])
            '''
            x, _ = self.bert(x, attention_mask = mask)
            y_pred = self.classify(x)
            # 将每个词表维度(y_pred.shape[-1],即:bert_token_vocab_size)与每个y(真实label),进行交叉熵计算loss
...
def train(corpus_path, save_weight=True):
    ...
    tokenizer = BertTokenizer.from_pretrained(r'D:\python\python_learn\python_learn\bert-base-chinese')
...
if __name__ == "__main__":
    ...
    train(r"D:\百度网盘\week10 文本生成\week10 文本生成问题\lstm语言模型生成文本\corpus.txt", False)
