...
class MultiClassificationModel(nn.Module):
    ...
    def forward(self, x, y=None):
        x = self.linear1(x)
        x = self.linear2(x)
        y_pred = self.linear3(x)  # (batch_size, input_size) -> (batch_size, 1)
        if y is not None:
...
def evaluate(model):
    ...
    with torch.no_grad():
        ...
        for y_p, y_t in zip(y_pred, y):  # 与真实标签进行对比
            print(f'evaluate,y_p={y_p},y_t={y_t}')
...
def main():
    ...
    input_size = 5     # 输入向量维度
    hidden_size1 = 10  # 隐含层1维度
    hidden_size2 = 20  # 隐含层2维度
    output_size = 5    # 输出层维度
    learning_rate = 0.001  # 学习率
    # 建立模型
    model = MultiClassificationModel(input_size, hidden_size1, hidden_size2, output_size)
    # 选择优化器
...
def predict(model_path, input_vec):
    input_size = 5
    model = MultiClassificationModel(input_size, 10, 20, 5)
