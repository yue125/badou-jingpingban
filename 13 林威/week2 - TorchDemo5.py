...
"""
作业：原先是2分类任务，设置一个多分类任务
以交叉熵为loss

基于pytorch框架编写模型训练
实现一个自行构造的找规律(机器学习)任务
规律：x是一个5维向量，哪个index的值最大，则输出该index
[1,2,3,4,5] 4
[2,5,2,1,3] 1
[9,0,1,2,3] 0
[1,2,1,5,3] ?
    ...
    with torch.no_grad():
        ...
        for y_p, y_t in zip(y_pred, y):  # 与真实标签进行对比
            if float(y_p) < 0.5 and int(y_t) == 0:
                ...
                correct += 1  # 正样本判断正确
            elif float(y_p) >= 1.5 and float(y_p) < 2.5 and int(y_t) == 2:
                correct += 1  # 正样本判断正确
            elif float(y_p) >= 2.5 and float(y_p) < 3.5 and int(y_t) == 3:
                correct += 1  # 正样本判断正确
            elif float(y_p) >= 3.5 and float(y_p) < 4.5 and int(y_t) == 4:
                correct += 1  # 正样本判断正确
            else:
                wrong += 1
