...
class TorchModel(nn.Module):
    def __init__(self, input_size, classes_num):
        ...
        self.linear = nn.Linear(input_size, classes_num)  # 线性层
...
def build_dataset(total_sample_num):
    ...
    for i in range(total_sample_num):
        vec, index = build_sample()
        vectors.append(vec)
        indexes.append(index)
    return torch.FloatTensor(vectors), torch.FloatTensor(indexes)
...
def evaluate(model):
    model.eval()
    test_sample_num = 100
    vectors, indexes = build_dataset(test_sample_num)

    correct, wrong = 0, 0
    with torch.no_grad():
        pred = model(vectors)  # 模型预测
        for y_p, y_t in zip(pred, indexes):  # 与真实标签进行对比
...
def main():
    ...
    # 建立模型
    model = TorchModel(input_size, classes_num)
    # 选择优化器
...
def predict(model_path, input_vec):
    input_size = 3
    model = TorchModel(input_size, classes_num=3)
