...
class TorchModel(nn.Module):
    def __init__(self, config):
        ...
        self.loss = nn.functional.cross_entropy     # 采用交叉熵计算损失
...
class StackGatedCNN(nn.Module):
    ...
    def forward(self, x):
        ...
        for i in range(self.num_layers):
            ...
            x = self.bn_after_ff[i](x)
            # feed forward层，使用两个线性层
            l1 = self.ff_liner_layer1[i](x)
            l1 = torch.relu(l1)     # bert中这里是gelu
            l2 = self.ff_liner_layer2[i](l1)        # 二层线性
