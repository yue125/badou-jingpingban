...
bert = BertModel.from_pretrained(r"D:\算法学习\八斗课程-精品班\第六周\bert-base-chinese", return_dict=False)
...
class DiyBert:
    ...
    def load_weights(self, state_dict):
        ...
        self.embeddings_layer_norm_weight = state_dict["embeddings.LayerNorm.weight"].numpy()   #768*768,打印出来怎么是768，那最后得到的不就是4*1吗？
        self.embeddings_layer_norm_bias = state_dict["embeddings.LayerNorm.bias"].numpy()   #4*768
        self.transformer_weights = []
        #transformer部分，有多层
        for i in range(self.num_layers):
            q_w = state_dict["encoder.layer.%d.attention.self.query.weight" % i].numpy()    #768*768
            q_b = state_dict["encoder.layer.%d.attention.self.query.bias" % i].numpy()  #4*768
            k_w = state_dict["encoder.layer.%d.attention.self.key.weight" % i].numpy()  #768*768
            k_b = state_dict["encoder.layer.%d.attention.self.key.bias" % i].numpy()    #4*768
            v_w = state_dict["encoder.layer.%d.attention.self.value.weight" % i].numpy()    #768*768
            v_b = state_dict["encoder.layer.%d.attention.self.value.bias" % i].numpy()  #4*768
            attention_output_weight = state_dict["encoder.layer.%d.attention.output.dense.weight" % i].numpy()  #768*768
            attention_output_bias = state_dict["encoder.layer.%d.attention.output.dense.bias" % i].numpy()  #4*768
            attention_layer_norm_w = state_dict["encoder.layer.%d.attention.output.LayerNorm.weight" % i].numpy()   #768*768
            attention_layer_norm_b = state_dict["encoder.layer.%d.attention.output.LayerNorm.bias" % i].numpy() #4*768
            intermediate_weight = state_dict["encoder.layer.%d.intermediate.dense.weight" % i].numpy()  #3072*768
            intermediate_bias = state_dict["encoder.layer.%d.intermediate.dense.bias" % i].numpy()  #4*3072
            output_weight = state_dict["encoder.layer.%d.output.dense.weight" % i].numpy()  #768*3072
            output_bias = state_dict["encoder.layer.%d.output.dense.bias" % i].numpy()  #4*768
            ff_layer_norm_w = state_dict["encoder.layer.%d.output.LayerNorm.weight" % i].numpy()    #768
            ff_layer_norm_b = state_dict["encoder.layer.%d.output.LayerNorm.bias" % i].numpy()  #768
        ...
        #pooler层
        self.pooler_dense_weight = state_dict["pooler.dense.weight"].numpy()    #768*768
        self.pooler_dense_bias = state_dict["pooler.dense.bias"].numpy()    #4*768
                                                                            #所有参数我都打印看了，跟我想的不太一样，但是不知道为啥。。。希望老师能解答一下
