    ...
    def get_datas(self):
        ...
        with open(self.path, 'r', encoding='utf-8') as f:
            ...
            for line in f:
                ...
                # print(row)
                # if len(row[1]) > self.config["max_length"]:
                #     self.config["max_length"] = len(row[1]) - 2
                if self.config["model_type"] == "bert":
                    input_id = self.tokenizer.encode(row[1], max_length=self.config["max_length"], pad_to_max_length=True)
                else:
