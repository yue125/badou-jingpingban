...
class DataGenerator:
    ...
    def load(self):
            ...
            for line in f:
                ...
                if isinstance(line, dict):
                    ...
                    for question in questions:
                        ...
                        input_id = torch.tensor(input_id, dtype=torch.long)  # 将索引列表转换为张量

                        self.knwb[self.schema[label]].append((input_id, self.schema[label]))
        ...
        if self.data_type == "train":
            for label, question_tuples in self.knwb.items():
               for anchor_tuple in question_tuples:
                    anchor = anchor_tuple[0]
                    try:
                         positive = random.choice([q[0] for q in question_tuples if not torch.equal(q[0], anchor[0])])
                    except IndexError:
    # 处理无法选择正样本的情况，例如跳过当前锚点或选择锚点本身作为正样本
                         positive = anchor  # 作为一种简单的处理方式，可以选择锚点本身作为正样本



                    negative_label = random.choice([l for l in self.knwb.keys() if l != label])
                    negative = random.choice(self.knwb[negative_label])[0]
