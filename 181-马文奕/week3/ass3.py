...
class TorchModel_try(nn.Module):
    def __init__(self, vector_dim, sentence_length, vocab):
        super(TorchModel_try, self).__init__()
        self.embedding = nn.Embedding(len(vocab), vector_dim)  # embedding层
        # 使用LSTM替代池化
        self.lstm = nn.LSTM(vector_dim, vector_dim, batch_first=True)
...
def build_model(vocab, char_dim, sentence_length):
    model = TorchModel_try(char_dim, sentence_length, vocab)
    return model
