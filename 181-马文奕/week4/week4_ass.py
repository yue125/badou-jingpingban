...
# 定义一个函数来获取从某个位置开始的所有可能词语
def get_words_from_index(sentence, index):
    words = []
    for i in range(index, len(sentence)):
        word = sentence[index:i+1]
        if word in Dict:
            words.append(word)
    return words if words else [sentence[index:index+1]]  # 如果没有匹配，返回单字

# 递归函数来遍历所有可能的分词路径
def find_all_combinations(sentence, index=0, current_combination=[], all_combinations=[]):
    if index >= len(sentence):
        all_combinations.append(current_combination.copy())
        return

    for word in get_words_from_index(sentence, index):
        current_combination.append(word)
        find_all_combinations(sentence, index+len(word), current_combination, all_combinations)
...
# 测试句子
sentence = "经常有意见分歧"
all_combinations = find_all_combinations(sentence)
