...
class Evaluator:
    def __init__(self, config, model, logger):
        ...
        _, self.train_data = load_data(config["train_data_path"], config)
    ...
    def eval(self, epoch):
        ...
        # self.config[f"acc_round_{epoch}"] = "%.2f%%" % (acc * 100)
        self.config[f"acc_round_{epoch}"] = acc
        self.config["accuracy"] = "%.2f%%" % (acc * 100)
        self.show_stats()
        return
