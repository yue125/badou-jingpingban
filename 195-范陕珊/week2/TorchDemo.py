...
class TorchModel(nn.Module):
    def __init__(self, input_size):
        ...
        self.linear_1 = nn.Linear(10, 5)  # 线性层
    ...
    def forward(self, x, y=None):
        ...
        y_pred = self.linear_1(x)  # (batch_size, input_size) -> (batch_size, 1)
...
def evaluate(model):
    ...
    for y_sample in y:
        ...
        if max_index in sample_data:
            sample_data[max_index] += 1
        else:
            sample_data[max_index] = 1
    for index, num in sample_data.items():
        print(f"本次预测集中共有{num}个{index}类样本")
    correct, wrong = 0, 0
    with torch.no_grad():
        y_pred = model(x)  # 模型预测
        print(f"y{y_pred} {y}")
