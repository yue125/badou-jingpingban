...
随机产生五维向量，使用交叉熵作为损失函数，预测最大值出现的位置
"""
...
def evaluate(model):
    ...
    with torch.no_grad():
        ...
        total += y.size(0)
        accaury = correct / total
    print("正确预测个数：%d, 正确率：%f" % (correct, accaury))
    return accaury
...
def predict(model_path, input_vec):
    ...
    for vec, pred in zip(input_vec, predict):
