...
class LanguageModel(nn.Module):
    def __init__(self):
        ...
        self.bert = BertModel.from_pretrained(r"D:\projects\AI\pythonProject\ai-learning\top-round2\week6\bert-base-chinese",
                                              return_dict = False)
...
def build_sample(window_size, tokenizer, corpus):
    ...
    x = tokenizer.encode_plus(window,
                            ...
                            add_special_tokens=True)["input_ids"] #将字转换成序号
...
#window_size 样本长度
#corpus 语料字符串
def build_dataset(sample_length, window_size, tokenizer, corpus):
    dataset_x = []
    dataset_y = []
    for i in range(sample_length):
        x, y = build_sample(window_size, tokenizer, corpus)
        dataset_x.append(x)
        dataset_y.append(y)
...
def generate_sentence(openings, model, window_size, tokenizer):
    ...
    with torch.no_grad():
        ...
        while pred_char != "\n" and len(openings) <= 30:
            openings += pred_char
            x = tokenizer.encode_plus(openings[-window_size:],
                                      max_length=12,
                                      pad_to_max_length=True,
                                      add_special_tokens=True)["input_ids"]  # 将字转换成序号
...
def train(corpus_path, save_weight=True):
    ...
    tokenizer = BertTokenizer.from_pretrained(
        r"D:\projects\AI\pythonProject\ai-learning\top-round2\week6\bert-base-chinese")
    print("文本词表模型加载完毕，开始训练")
    for epoch in range(epoch_num):
        model.train()
        watch_loss = []
        for batch in range(int(train_sample / batch_size)):
            x, y = build_dataset(batch_size, window_size, tokenizer, corpus) #构建一组训练样本
            if torch.cuda.is_available():
                x, y = x.cuda(), y.cuda()
