...
def build_sample(vocab,sentence_length):
    #这里用sample是不放回的采样，每个字母不会重复出现，但是要求字符串长度要小于词表长度
...
#建立数据集，输入需要的样本数量，需要多少生成多少
def build_dataset(sample_length,vocab,sentence_lenth):
    dataset_x=[]
    dataset_y=[]
    for i in range(sample_length):
        x,y=build_sample(vocab,sentence_lenth)
        dataset_x.append(x)
        dataset_y.append(y)
...
if __name__ == '__main__':
    main()
    test_strings= ["bacdefghijk", "bcadefghijk", "bcdaefghijk", "bcdeafghijk", "bcdefaghijk", "bcdefgahijk", "bcdefghaijk", "bcdefghiajk", "bcdefghijak", "bcdefghijka"]
