...
class SentenceLabel:
    ...
    def predict(self, sentence):
        ...
        with torch.no_grad():
            ...
            res = torch.argmax(torch.tensor(res), dim=-1)  # 将res转换为张量后再调用argmax()
        labeled_sentence = ""
        for char, label_index in zip(sentence, [res.item()]):  # 将res包装在列表中
...
if __name__ == "__main__":
    ...
    sentence = "在北京，李华教授参观了清华大学"
    res = sl.predict(sentence)
    print(res)

    sentence = "伦敦的白金汉宫是伊丽莎白女王的居所，约翰·史密斯先生在那里举行了一场重要的会议。"
    res = sl.predict(sentence)
    print(res)
