...
def build_sample(vocab, window_size, corpus):
    ...
    # print(window, target)
    x = vocab.encode(window,padding="max_length",
                      max_length=10,
                      truncation=True)  #将字转换成序号
    y = vocab.encode(target,padding="max_length",
                      max_length=10,
                      truncation=True)
    return x, y
...
def generate_sentence(openings, model, vocab, window_size):
    ...
    with torch.no_grad():
        ...
        while pred_char != "\n" and len(openings) <= 30:
            openings += pred_char
            x = vocab.encode(openings, add_special_tokens=False)  # 将字转换成序号
            x = torch.LongTensor([x])
            if torch.cuda.is_available():
                x = x.cuda()
            y = model(x)[0][-1]
            index = sampling_strategy(y)
            pred_char = ''.join(vocab.decode(index))
    return openings
