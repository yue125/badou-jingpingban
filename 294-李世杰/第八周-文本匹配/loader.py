...
class DataGenerator:
            ...
            for line in f:
                ...
                if isinstance(line, dict):
                    ...
                    for question in questions:
                        if self.config["bert"]:
                            input_id = self.tokenizer.encode(question, max_length=self.config["max_length"],pad_to_max_length=True,truncation=True)
                        else:
    ...
    def random_train_sample(self):
        ...
        if self.config['loss_type'] == "cosine_loss":
            #随机正样本
            if random.random() <= self.config["positive_sample_rate"]:
                ...
                if len(self.knwb[p]) < 2:
                    ...
                    return [s1, s2, None, torch.LongTensor([1])]
            #随机负样本
            else:
                ...
                return [s1, s2, None, torch.LongTensor([-1])]
        elif self.config['loss_type'] == "triple_loss":
            p, n = random.sample(standard_question_index, 2)
            #如果选取到的标准问下不足两个问题，则无法选取，所以重新随机一次
            if len(self.knwb[p]) < 2:
                return self.random_train_sample()
