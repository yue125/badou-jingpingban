...
class TorchModel(nn.Module):
    def __init__(self, vector_dim, sentence_length, vocab):
        print(vector_dim, sentence_length, vocab)
...
#如果存在字符a，返回a的索引，不存在则随机位置替换为a，保证字符串始终存在a
...
def predict(model_path, vocab_path, input_strings):
    ...
    for i, input_string in enumerate(input_strings):
        print("输入：%s, 预测类别：%d, 概率值：%s" % (input_string, torch.argmax(result[i]), result[i])) #打印结果



if __name__ == "__main__":
    main()
    test_strings = ["anvfee", "wzsafg", "rqwdea", "nakwew"]
