...
规律：x是一个5维向量，第i维最大，类别为第i类


总结：
1.多分类使用softmax激活函数
2.nn.crossEntropy损失函数自带了softmax过程。其输入的input shape (batch_size,num_class).target shape (batch_size)为1Dtensor，且要求数据类型为int64
3.argmax返回最大值下标
...
class TorchModel(nn.Module):
    def __init__(self, input_size):
        ...
        self.linear = nn.Linear(input_size, 5)  # 线性层,五分类映射到5列
        self.activation = torch.softmax  # sigmoid归一化函数,softmax多用于多分类函数
    ...
    def forward(self, x, y=None):
        ...
        #这里没有使用激活函数，因为多分类采用softmax激活函数，而torch.nn.cross_entropy自带了softmax
        y_pred=x

        if y is not None:
            y=torch.reshape(y,[x.shape[0]])#损失函数对输入的target形状有要求。
            y=y.long()#损失函数对数据类型有要求
