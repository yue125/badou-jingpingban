...
基于pytorch框架编写模型训练-作业
实现一个自行构造的找规律(机器学习)任务-作业
规律：x是一个5维向量，第几个数最大就返回几,从索引0开始
...
def evaluate(model):
    ...
    print("本次预测集中共有%d个第一类，%d个第二类，%d个第三类，%d个第四类，%d个第五类" % (torch.flatten(y).tolist().count(0),
                                             torch.flatten(y).tolist().count(1),torch.flatten(y).tolist().count(2),
                                             torch.flatten(y).tolist().count(3),torch.flatten(y).tolist().count(4)))
    correct, wrong = 0, 0
    with torch.no_grad():
        y_pred = model(x)  # 模型预测
