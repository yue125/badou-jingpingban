...
class TorchModel(nn.Module):
    ...
    def forward(self, x, y=None):
        ...
        if y is not None:
            return nn.functional.cross_entropy(y_pred, y.squeeze().long())
        else:
            return y_pred
...
def evaluate(model, vocab, sample_length):
    ...
    class_counts = [0] * 6
    for label in y:
        class_counts[label - 1] += 1  # 标签从1到6
    print("本次预测集中每个类别的样本数:", class_counts)

    correct, wrong = 0, 0
    with torch.no_grad():
        y_pred = model(x)  # 模型预测
        predicted_labels = torch.max(y_pred, 1)[1]  # 获取概率最高的类别
