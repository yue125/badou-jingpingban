...
# 创建训练集，正常任务是读取训练集
iris = datasets.load_iris()  # 加载水仙花数据集
# 数据集分割
X_train1, X_test1, y_train1, y_test1 = train_test_split(iris.data, iris.target, test_size=0.2, random_state=42)
# 转换为张量
...
# 评估代码
# 用来测试每轮模型的准确率 随机在训练集中抽取20个
def evaluate(model):
    model.eval()
    print("随机抽取训练集中20个样本")
    sample_size = 20 # 每次抽取20组
