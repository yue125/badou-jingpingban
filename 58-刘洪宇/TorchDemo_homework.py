...
class TorchModel(nn.Module):
    ...
    def forward(self, x, y=None):
        l1 = self.layer1(x)  # (batch_size, input_size) -> (batch_size, 1)
        l2 = self.layer2(l1)
        x = self.layer3(l2)
...
def evaluate(model):
    ...
    print("本次预测集中共有%d个一类样本，%d个二类样本，%d个三类样本，" % (sum([x[0] for x in y]), sum([x[1] for x in y]), sum([x[2] for x in y])))
    correct, wrong = 0, 0
    with torch.no_grad():
        y_pred = model(x)  # 模型预测
        # print(y_pred)
        for y_p, y_t in zip(y_pred, y):  # 与真实标签进行对比
            if float(y_p[0]) == max(y_p) and int(y_t[0]) == 1:
                correct += 1  # 负样本判断正确
            elif float(y_p[1]) == max(y_p) and int(y_t[1]) == 1:
                correct += 1  # 正样本判断正确
            elif float(y_p[2]) == max(y_p) and int(y_t[2]) == 1:
                correct += 1  # 正样本判断正确
            else:
                wrong += 1
...
def predict(model_path, input_vec):
    ...
    for vec, res in zip(input_vec, result):
        print("输入：{0}, 预测类别：{1}, 概率值：{2}" .format (vec, [x+1 for x in range(len(res)) if res[x] == max(res)], res))  # 打印结果
