...
def evaluate(model):
    ...
    with torch.no_grad():
        ...
        for y_p, y_t in zip(y_pred, y):  # 与真实标签进行对比
            if y_p[0] < y_p[1] and y_p[0] <= y_p[4] and int(y_t) == 0:
                correct += 1  # 负样本判断正确
            elif y_p[1] >= y_p[0] and y_p[1] >= y_p[4] and int(y_t) == 2:
                correct += 1  # 正样本判断正确
            elif y_p[0] > y_p[4] and y_p[0] < y_p[1] and int(y_t) == 1:
                correct += 1  # 正样本判断正确
            else:
                wrong += 1
...
def predict(model_path, input_vec):
    ...
    for vec, res in zip(input_vec, result):
        type_result = get_type(vec)
        print("输入：%s, 预测类别：%s, 概率值：%s" % (vec, type_result, res))  # 打印结果
