...
class LanguageModel(nn.Module):
    def __init__(self, input_dim, vocab_size, pretrain_model_path):
...
def sampling_strategy(prob_distribution):
    ...
    if strategy == "greedy":
        ...
        return np.random.choice(
            list(range(len(prob_distribution))), p=prob_distribution
        )
...
def train(corpus_path, save_weight=True):
    ...
    pretrain_model_path = r"D:\Learn\AI\bert-base-chinese"
    ...
    for epoch in range(epoch_num):
        ...
        for batch in range(int(train_sample / batch_size)):
            x, y = build_dataset(
                batch_size, tokenizer, window_size, corpus
            )  # 构建一组训练样本
            if torch.cuda.is_available():
                x, y = x.cuda(), y.cuda()
        ...
        print(
            generate_sentence(
                "让他在半年之前，就不能做出", model, tokenizer, window_size
            )
        )
        print(
            generate_sentence(
                "李慕站在山路上，深深的呼吸", model, tokenizer, window_size
            )
        )
    if not save_weight:
        return
