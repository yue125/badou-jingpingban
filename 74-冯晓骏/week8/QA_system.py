...
from siamese_load import SiameseLoader
...
class QASystem:
    ...
    def query(self, user_query):
        ...
        elif self.algo == 'siamese':
            query_vector = self.siamese_loader.get_vector(user_query)
            print('q:',query_vector.shape)
            for target,vector in self.vectors.items():
                if len(vector.shape) ==1:
                    vector = vector.unsqueeze(0)
                #求余弦相似度
                coses = torch.mm(query_vector,vector.T)
                results.append([target,torch.mean(coses)])
        else:
    ...
    def load_siamese(self):
        self.siamese_loader = SiameseLoader()

    def load_all_vectors(self):
        self.vectors = {}
        for target, questions in self.target_to_questions.items():
            self.vectors[target] = self.siamese_loader.get_vectors(questions)
        # print(self.vectors)


if __name__ == '__main__':
    qas = QASystem("data/train.json", "siamese")
    # question = "我想重置下固话密码"
    question = "什么是亲情号"
