...
class Evaluator:
    ...
    def eval(self, epoch):
        ...
        for index, batch_data in enumerate(self.valid_data):
            # print('batch_data:', batch_data[0].shape)
            if torch.cuda.is_available():
                batch_data = [d.cuda() for d in batch_data]
            input_id, labels = batch_data   #输入变化时这里需要修改，比如多输入，多输出的情况
            with torch.no_grad():
                test_question_vectors = self.model(input_id) #不输入labels，使用模型当前参数进行预测
                # print('test_question_vectors:', test_question_vectors.shape)
                # print('test_question_vectors.unsqueeze:', test_question_vectors.unsqueeze(0).shape)
    ...
    def write_stats(self, test_question_vectors, labels):
        ...
        for test_question_vector, label in zip(test_question_vectors, labels):
            ...
            # print('quest:',test_question_vector.unsqueeze(0).shape)
            # print('knwb T:',self.knwb_vectors.T.shape)
            res = torch.mm(test_question_vector.unsqueeze(0), self.knwb_vectors.T)
            # print('res:',res.shape)
            # i = torch.argmax(torch.sum(self.knwb_vectors, dim=1),dim=-1)
            # print('i:',int(i))
