...
规律：x是一个5维向量，向量中哪个标量最小就输出哪一维的下标
...
# 随机生成一个5维向量，根据每个向量中最小的标量同一下标构建Y
def build_sample():
...
def evaluate(model):
    ...
    with torch.no_grad():
        ...
        for y_p, y_t in zip(y_pred, y):  # 与真实标签进行对比
            if torch.argmin(y_p) == int(y_t):
                correct += 1  # 样本为标量中最小值，判断正确
            else:
                wrong += 1
...
def predict(model_path, input_vec):
    ...
    for vec, res in zip(input_vec, result):
        print("输入：%s, 预测类别：%s, 概率值：%s" % (vec, torch.argmin(res), res))  # 打印结果


if __name__ == "__main__":
    main()
    test_vec = [[0.17889086, 0.15229675, 0.31082123, 0.03504317, 0.08920843],
                [0.24963533, 0.5524256, 0.95758807, 0.93520434, 0.84890681],
                [0.58797868, 0.27482528, 0.13625847, 0.34675372, 0.19871392],
                [0.49349776, 0.39416669, 0.12579291, 1.41567412, 0.7358894]]
