...
#建立模型
def build_model(tokenizer, char_dim, bert_path):
    model = LanguageModel(char_dim, tokenizer, bert_path)
    return model

#文本生成测试代码
def generate_sentence(openings, model, tokenizer, window_size):
    # reverse_vocab = dict((y, x) for x, y in tokenizer.items())
    model.eval()
    with torch.no_grad():
        pred_char = ""
...
def train(corpus_path, save_weight=True):
    ...
    model = build_model(tokenizer, char_dim, pretrain_model_path)    #建立模型
    if torch.cuda.is_available():
        model = model.cuda()
