...
class TorchModel(nn.Module):
    ...
    def forward(self, x, y=None):
        ...
        x = rnn_out[:, -1, :]
        y_pred = self.classify(x)                       #(batch_size, vector_dim) -> (batch_size, 5) 3*5 5*5 -> 3*5
        if y is not None:
...
def main():
    ...
    # 保存词表
    writer = open("vocab1.json", "w", encoding="utf8")
...
if __name__ == "__main__":
    main()
    test_strings = ["afcbedhg", "ibcdjfgh", "dbcaefjh", "fbcdeagh"]
    predict("model1.pth", "vocab1.json", test_strings)
