...
def evaluate(model):
    ...
    print("本次预测集中各类别样本数量：", [sum(y == i) for i in range(num_classes)])
    correct, wrong = 0, 0
    with torch.no_grad():
        y_pred = model(x)  # 模型预测
...
def predict(model_path, input_vec):
    ...
    for vec, res in zip(input_vec, result):
        print("输入：%s, 预测类别：%d, 预测概率值：%s" % (vec, torch.argmax(res, dim=0), res))  # 打印结果
...
if __name__ == "__main__":
    ...
    test_vec = [[0.07889086,0.15229675,0.31082123,0.03504317,0.18920843],
                ...
                [0.19349776,0.59416669,0.92579291,0.41567412,0.111111111111111111111111111111111111]]
